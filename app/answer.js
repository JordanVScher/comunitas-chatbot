...
module.exports.handleText = async (context, apiai, sheetAnswers) => {
	try {
		...
		await Raven.captureException(error,
			{
				user: {
					username: context.session.user.first_name, whatWasTyped: context.state.whatWasTyped, apiaiResp: context.state.apiaiResp, function: 'handleText',
				},
			});
	}
};
...
module.exports.handleQuestionQuickReply = async (context, sheetAnswers) => {
	try {
		...
		await Raven.captureException(error,
			{
				user: {
					username: context.session.user.first_name, QRPayload: context.state.questionID, apiaiResp: context.state.apiaiResp, function: 'handleQuestionQuickReply',
				},
			});
	}
};
...
module.exports.sendAnswerInSheet = async (context, question) => {
	try {
		...
		await Raven.captureException(error,
			{
				user: {
					username: context.session.user.first_name, whatWasTyped: context.state.whatWasTyped, currentAnswer: context.state.currentAnswer, function: 'sendAnswerInSheet',
				},
			});
	}
};
...
module.exports.sendRelatedQuestions = async (context, sheetAnswers, question) => {
	try {
		...
		await Raven.captureException(error,
			{
				user: {
					username: context.session.user.first_name, currentAnswer: context.state.currentAnswer, function: 'sendRelatedQuestions', related: context.state.related,
				},
			});
	}
};
